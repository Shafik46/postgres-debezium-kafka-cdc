...
In PostgreSQL, that change log is the Write-Ahead Log, usually called WAL. Every insert, update, and delete is first recorded in WAL so PostgreSQL can recover safely after crashes. When `wal_level=logical` is enabled, Debezium can read those WAL entries through logical replication and convert them into CDC events.

In this project, the simulator updates PostgreSQL tables, PostgreSQL records those changes in WAL, Debezium reads the WAL stream, and Kafka stores the resulting CDC events as JSON messages.
...
- `simulator`: Python app that generates and updates order data
...
- [ops_lifecycle_simulator.py](./simulator/ops_lifecycle_simulator.py): generates insert and update events
...
1. The simulator acts like a transactional application and writes order data into PostgreSQL.
2. PostgreSQL stores that source data in `ops_sys.order_lifecycle` and `ops_sys.order_events`.
3. PostgreSQL also records those row changes in WAL.
4. Debezium reads WAL through logical replication and publishes the changes into Kafka topics.

That means the rows inside PostgreSQL are the source data, while the JSON messages you see in Kafka are the CDC events produced from those database changes.
...
6. PostgreSQL records those inserts and updates in WAL.
7. Debezium reads the WAL stream through logical replication and converts the row changes into Kafka messages.
...
python3 simulator/ops_lifecycle_simulator.py --new-orders 10 --update-cycles 10 --sleep-seconds 3
```
